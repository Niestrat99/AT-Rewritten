...
public class PaymentManager {
    ...
    private void addCommand(String command, Object value) {
        ...
        for (String rawPayment : rawPayments) {
            try {
                ...
                if (payment == null) {
                    CoreAdvancedTeleport.debug("Payment " + rawPayment + " did not have a payment formed.");
                    continue;
                }
                CoreAdvancedTeleport.debug("Payment " + rawPayment + " for " + command + " was parsed as a " + payment.getClass().getSimpleName());
    ...
    private Payment parsePayment(String rawPayment) {
        if (rawPayment.length() - 3 <= 0) {
            ...
            if (matcher.matches()) {
                ...
                CoreAdvancedTeleport.debug("Split payment into two, part 1: " + plugin + ", part two: " + payment);
                CoreAdvancedTeleport.debug("Material: " + Material.getMaterial(plugin == null ? "" : plugin));
    ...
    public boolean canPay(String command, Player player, World toWorld) {
        CoreAdvancedTeleport.debug("Verifying that " + player.getName() + " can pay to run " + command + " into " + toWorld.getName());
        if (player.hasPermission("at.admin.bypass.payment")) {
            CoreAdvancedTeleport.debug(player.getName() + " has at.admin.bypass.payment - not checking payments.");
            return true;
        }
        for (Payment payment : getPayments(command, player, toWorld).values()) {
            if (!payment.canPay(player)) {
                CoreAdvancedTeleport.debug(player.getName() + " failed to afford payment " + payment.getId() + ".");
                return false;
            }
        }
        return true;
