...
public class RTPManager {
    ...
    public static void init() {
        ...
        if (!PaperLib.isPaper()) {
            CoreAdvancedTeleport.debug("Server is not using Paper, cannot initialise RTPManager.");
            return;
        }
        if (!MainConfig.get().RAPID_RESPONSE.get()) {
            CoreAdvancedTeleport.debug("Rapid response is not enabled, cannot initialise RTPManager.");
            return;
        }
    ...
    public static CompletableFuture<@Nullable Location> addLocation(
        ...
        CoreAdvancedTeleport.debug("Attempting to either add a location or return it for /rtp.");
        ...
        if (locQueue.get(world.getUID()) != null
            ...
            if (!PluginHookManager.get().isClaimed(loc)) {
                CoreAdvancedTeleport.debug("Area is not claimed - returning as valid location");
        ...
        CoreAdvancedTeleport.debug("Attempting coordinates: " + Arrays.toString(coords) + " (try " + finalTries + ")");
                ...
                .thenApplyAsync(
                        chunk -> {
                            ...
                            if (isValidLocation(block)) {
                                CoreAdvancedTeleport.debug("Valid location found - returning it");
                                return block.getLocation().add(0.5, 1, 0.5);
                            } else if (finalTries < 10) {
                                CoreAdvancedTeleport.debug("Invalid location, trying again...");
                                return addLocation(world, urgent, finalTries).join();
                            } else {
                                CoreAdvancedTeleport.debug("Invalid location - not going to try again");
                                return null;
                            }
                        },
                        CoreAdvancedTeleport.async)
                .thenApplyAsync(loc -> loc, CoreAdvancedTeleport.sync);
    }
    ...
    public static void getPreviousLocations() throws IOException {
        CoreAdvancedTeleport.debug("Loading previously discovered locations.");
        File rtpLocsFile = new File(CoreAdvancedTeleport.getInstance().getPlugin().getDataFolder(), "rtp-locations.csv");
        if (!rtpLocsFile.exists()) {
            CoreAdvancedTeleport.debug("Not loading previous locations - rtp-locations.csv does not exist.");
            return;
        }
        ...
        while ((currentLine = reader.readLine()) != null) {
            try {
                ...
                CoreAdvancedTeleport.debug("Added previous location " + location);
