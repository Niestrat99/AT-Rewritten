...
public class TeleportTrackingManager implements Listener {
    ...
    public void onJoin(PlayerJoinEvent e) {
        ...
        if (!player.hasPermission("at.admin.bypass.teleport-on-join")) {
            ...
            if (!player.hasPlayedBefore() && MainConfig.get().TELEPORT_TO_SPAWN_FIRST.get()) {
                loc = SpawnConfig.get().getSpawn(MainConfig.get().FIRST_SPAWN_POINT.get(), player, true);
            } else if (MainConfig.get().TELEPORT_TO_SPAWN_EVERY.get()) {
                loc = SpawnConfig.get().getSpawn(e.getPlayer().getWorld().getName(), player, false);
    ...
    public void onRespawn(PlayerRespawnEvent e) {
        ...
        if (MainConfig.get().USE_SPAWN.get()) {
    ...
    private static boolean handleSpawn(PlayerRespawnEvent e, String spawnCommand) {
        ...
        switch (spawnCommand) {
            case "spawn":
                Location spawn = SpawnConfig.get().getSpawn(e.getPlayer().getWorld().getName(), e.getPlayer(), false);
                if (spawn != null) {
