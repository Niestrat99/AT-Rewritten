...
public final class CustomMessages extends ATConfig {
    ...
    public static void sendMessage(
        ...
        if (sender instanceof Player player) {
            ...
            handleSpecialMessage(
                    player,
                    path + "_actionbar",
                    (content -> asAudience(player).sendActionBar(translate(content, placeholders))),
                    actionBarManager);
            handleSpecialMessage(
                    player,
                    path + "_sound",
    ...
    private static void sendSound(
        ...
        if (rawSound[0].matches("([a-z0-9_\\-.]+:)?[a-z0-9_\\-./]+")) {
            try {
                player.playSound(
                        Sound.sound(Key.key(rawSound[0]), Sound.Source.NEUTRAL, volume, pitch));
                ...
                CoreClass.getInstance()
                        ...
                        .warning(
                                "Sound for "
                                        ...
                                        + " ("
                                        + rawSound[0]
                                        + ") could not be played: namespaces are not supported on your platform.");
            }
        } else {
            try {
