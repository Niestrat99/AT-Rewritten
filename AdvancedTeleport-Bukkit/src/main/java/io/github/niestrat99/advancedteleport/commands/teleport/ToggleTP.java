...
public final class ToggleTP extends TeleportATCommand implements PlayerCommand {
    ...
    public boolean onCommand(
        ...
        // If more than one argument has been specified, see if the sender can change the
        // teleportation state of another player
        if (args.length > 0 && sender.hasPermission("at.admin.toggletp")) {
            ...
            if (atPlayer.isTeleportationEnabled()) {
                ...
                atPlayer.setTeleportationEnabled(true, sender)
                        .thenAcceptAsync(
                                result -> {
        ...
        if (atPlayer.isTeleportationEnabled()) {
            ...
            atPlayer.setTeleportationEnabled(true, sender)
                    .thenAcceptAsync(callback -> CustomMessages.sendMessage(sender, "Info.tpOn"));
        }
